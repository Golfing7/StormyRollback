...
public class RollbackListMenu {
	...
	public RollbackListMenu(Player staff, OfflinePlayer player, LogType logType, int pageNumber) {
		...
		this.playerData = InventoryRollback.getInstance().getPlayerData(playerUUID).getData(logType);
	}
	...
	public Inventory showBackups() {						
		...
		for (int i = 0; i < spaceRequired; i++) {							
			try {
				...
				if(playerData.contains("data." + time + ".killer"))
					lore.add(messages.killer(playerData.getString("data." + time + ".killer")));
